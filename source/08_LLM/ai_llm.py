...
#  재사용 가능한 함수로 만들기
def ask_with_reference_rerank(query: str, chat_historhy: list=None, k: int = 15, top_k:int=4, ):
    """
    ...
    if chat_historhy:
        history_text = [f"{'사용자' if msg['role']=='user' else 'AI'}: {msg['content']}" for msg in chat_historhy]
        history_text = "\n".join(history_list)
