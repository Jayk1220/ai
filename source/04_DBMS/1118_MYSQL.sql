...
-- DDL (제약조건 FK, 시퀀스 없음)
-- DML (AND : &&, OR: ||, 연결연산자 대신 CONCAT 사용, OUTER JOIN)
...
grant all privileges on *.* to user01; -- 권한부여 *.* --> "모든 스키마"."모든 권한"
...
/* MySQL 타입: numeric(n, d), varchar(n)
...
select empno "no", ename 이름, nickname as "별명", sal, comm from emp;


-- 시퀸스 대체, fk제약조건 
-- major : (mcode시퀀스, moffice)/ student(sno, sname, mcode:FK)
-- set @@auto_increment_increment=10

create table major(
	mcode int primary key auto_increment, -- auto_increment 필드(int)
    mname varchar (10),
    moffice varchar (10)    
);
...
-- FK(외래키) 제약조건은 아래에 기술해야 함
drop table if exists major;
drop table if exists student;

create table major(
	mcode int auto_increment, -- auto_increment 필드(int)
    mname varchar (10),
    moffice varchar (10),
    primary key(mcode)
);
...
create table division(
    ...
    phone varchar(20) unique, -- 부서 전화번호
...
-- workbench에서 자동 commit 모드 운용 (싫으면 edit preferencce에서 바꾸기)
select * from division;
select * from personal;


-- 1. 사번, 이름, 급여를 출력
select pno, pno, pay from personal;
...
select pno, pname, dname
	from personal left join division
    on personal.dno = division.dno;

-- 7. 사번, 이름, 상사이름
select e.pno, e.pname, b.pname
	from personal e, personal b
    where e.manager = b.pno;

-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) 
select e.pno, e.pname, ifnull(b.pname, 'ceo') manager
	from personal e left outer join personal b
    on e.manager = b.pno;
...
select e.pno, e.pname, b.pname, dname
	from personal e left outer join personal b on e.manager=b.pno 
					left outer join division d on e.dno = d.dno;
...
select e.pno, e.pname, e.pay, d.dname, b.pname
	from personal e
		left outer join personal b on e.manager = b.pno
        left outer join division d on e.dno = d.dno;
        
-- oracle과 다른 함수들과 연결연산자 (||)
select pname||'은'||job from personal; -- 연결연산 안됨
select concat(pname, '은', job) from personal;

select sysdate(); -- mysql은 select 절 가능
-- date_format(날짜형, 포맷) : 날짜형을 문자형으로
-- date_format(문짜형, 포맷) : 문자형을 날짜형으로
	-- %Y (년도4자리) %y (년도2자리) %m (월) %d (일 01,02,03,...), %c (일 1,2,3)
    -- %h (시) %H (24시간 시) %i (분) %s (초) %p (오전오후)
