...
--사번 - 부서번호, 부서명, 부서위치
SELECT * FROM EMP, DEPT WHERE ENAME='SCOTT';  --무의미한 조인
...
-- 급여가 2000 이상인 직원의 이름 직책 급여 부서명 근무지

SELECT ENAME, JOB, SAL, DNAME, LOC FROM EMP E, DEPT D; --ㄷ테이블 14개 * D테이블 4개
...
SELECT * FROM DEPT;    
    --EX 근무지(LOC)가 CHICAGO인 사람의 이름, 급여, 부서번호 출력
    ...
    -- EX COMM이 NULL이고 SAL이 1200이상인 사원의 이름 급여 부서번호 부서명
        ...
        FROM EMP E, DEPT D
        WHERE D.DEPTNO = E.DEPTNO AND (COMM IS NULL OR COMM = 0) AND SAL >=1200
...
SELECT * FROM EMP, SALGRADE WHERE ENAME = 'SCOTT' AND SAL BETWEEN LOSAL AND HISAL;
    ...
    -- EX 모든 사원의 사번, 이름, 급여 급여등급 (1등급, 2등급...),상사사번
...
SELECT ENAME, SAL, GRADE, EMP.DEPTNO, DNAME, LOC
    ...
    WHERE EMP.DEPTNO=DEPT.DEPTNO AND COMM IS NOT NULL AND COMM != 0 AND SAL BETWEEN LOSAL AND HISAL;
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER.ENAME 매니저
    ...
    --EX. 'SMITH 의 상사는 FORD 다' FORMAT으로 출력
...
-- 4. OUTER JOIN : SELF-JON시 조건에 만족하지 않는 행까지 나타나게
...
SELECT W.ENAME, W.MGR, M.EMPNO, M.ENAME
    ...
    --EX 'SMITH'의 상사는 FORD' - KING의 상사는 없다 
...
Part2
--1. 이름, 직속상사명
SELECT E.ENAME EMPLOYEE, S.ENAME SUPERVISOR FROM EMP E, EMP S
    WHERE E.MGR = S.EMPNO ;
    
--2. 이름, 급여, 업무, 직속상사명
SELECT E.ENAME, E.SAL, E.JOB, S.ENAME SUPERVISOR 
    FROM EMP E, EMP S
    WHERE E.MGR = S.EMPNO;

--3. 이름, 급여, 업무, 직속상사명 . (상사가 없는 직원까지 전체 직원 다 출력. 상사가 없을 시 '없음'으로 출력)
SELECT E.ENAME, E.SAL, E.JOB, NVL(S.ENAME, '없음') SUPERVISOR
    FROM EMP E, EMP S
    WHERE E.MGR = S.EMPNO(+);

--4. 이름, 급여, 부서명, 직속상사명
SELECT E.ENAME, E.SAL, D.DNAME, S.ENAME
...
SELECT E.ENAME, E.SAL, D.DNAME, LOC, NVL(S.ENAME,'없음')
    FROM EMP E, EMP S, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
        AND E.MGR = S.EMPNO(+);
        
--6. 이름, 급여, 등급, 부서명, 직속상사명. 급여가 2000이상인 사람
SELECT E.ENAME, E.SAL, GRADE, DNAME, S.ENAME
    FROM EMP E, EMP S, DEPT D, SALGRADE G
    WHERE E.DEPTNO = D.DEPTNO
...
SELECT E.ENAME, E.SAL, GRADE, DNAME, S.ENAME
    FROM EMP E, EMP S, SALGRADE G, DEPT D
    WHERE E.DEPTNO = D.DEPTNO
        AND E.SAL BETWEEN LOSAL AND HISAL
        AND E.MGR = S.EMPNO(+)
    ORDER BY DNAME;
...
SELECT E.ENAME, E.SAL, GRADE, DNAME, (E.SAL+NVL(E.COMM,0))*12 ANNUAL
    FROM EMP E, EMP S, DEPT D, SALGRADE
    WHERE E.MGR = S.EMPNO
...
SELECT E.ENAME, S.ENAME 상사, B.ENAME "상사의 상사"
    FROM EMP E, EMP S, EMP B
    WHERE E.MGR = S.EMPNO
        AND S.MGR = B.EMPNO;
    

--11. 위의 결과에서 상위 상사가 없는 모든 직원의 이름도 출력되도록 수정하시오(outer join)
SELECT E.ENAME, NVL(S.ENAME,'없음') 상사, NVL(B.ENAME,'없음') "상사의 상사"
    FROM EMP E, EMP S, EMP B
    WHERE E.MGR = S.EMPNO
        AND S.MGR = B.EMPNO(+);